...
class vg_decoder_wrapper(nn.Module):
    ...
    def forward(self, img_feat, mask, pos_embed, cate_asso_sent1, cate_asso_sent2, cate_asso_sent3, word_mask):
        hs = self.decoder(img_feat, mask, pos_embed, cate_asso_sent1, cate_asso_sent2, cate_asso_sent3, word_mask)
...
class DecoderWithExtraEncoder(nn.Module):
    def __init__(self, num_queries, query_dim,
        ...
        self.merge = MLP(256*3, 256*2, 256, 2)

    def with_pos_embed(self, tensor, pos: Optional[Tensor]):
        return tensor if pos is None else tensor + pos

    def forward(self, img_feat, img_key_padding_mask=None, pos=None, cate_asso_sent1=None, cate_asso_sent2=None, cate_asso_sent3=None, word_key_padding_mask=None):
        intermediate = []
        hw, bs, c = img_feat.shape

        # Knowledge accumulation
        for layer in self.extra_encoder_layers:
            img_feat = layer(img_feat, img_key_padding_mask, pos, cate_asso_sent1, cate_asso_sent2, cate_asso_sent3, word_key_padding_mask, None)
        ...
        word_feat = self.merge(torch.cat([cate_asso_sent1, cate_asso_sent2, cate_asso_sent3], dim=-1))
...
class DiscriminativeFeatEncLayer(nn.Module):
    def __init__(self, d_model, img2text_attn_args=None, img_query_with_pos=True,
        ...
        atn = args.pop('type')
        self.img2text_attn_fact1 = MULTIHEAD_ATTNS[atn](**args)
        self.img2text_attn_fact2 = MULTIHEAD_ATTNS[atn](**args)
        self.img2text_attn_fact3 = MULTIHEAD_ATTNS[atn](**args)
        self.img_query_with_pos = img_query_with_pos

        self.text_proj_fact1 = MLP(**vl_verify['text_proj'])
        self.text_proj_fact2 = MLP(**vl_verify['text_proj'])
        self.text_proj_fact3 = MLP(**vl_verify['text_proj'])
        ...
        self.tf_scale_fact2 = Parameter(torch.Tensor([vl_verify.get('scale')]))
        self.tf_sigma_fact2 = Parameter(torch.Tensor([vl_verify.get('sigma')]))
        self.tf_scale_fact3 = Parameter(torch.Tensor([vl_verify.get('scale')]))
        self.tf_sigma_fact3 = Parameter(torch.Tensor([vl_verify.get('sigma')]))

        args = img2textcond_attn_args.copy()
        atn = args.pop('type')
        self.img2textcond_attn_fact1 = MULTIHEAD_ATTNS[atn](**args)
        self.img2textcond_attn_fact2 = MULTIHEAD_ATTNS[atn](**args)
        self.img2textcond_attn_fact3 = MULTIHEAD_ATTNS[atn](**args)
        self.merge = MLP(256*3, 256*2, 256, 2)
    ...
    def forward(self, img_feat, img_key_padding_mask, img_pos,
                cate_asso_sent1, cate_asso_sent2, cate_asso_sent3, word_key_padding_mask, word_pos=None):
        orig_img_feat = img_feat

        # 1st branch
        img_query = img_feat + img_pos if self.img_query_with_pos else img_feat
        text_info_fact1 = self.img2text_attn_fact1(query=img_query, key=self.with_pos_embed(cate_asso_sent1, word_pos), value=cate_asso_sent1, key_padding_mask=word_key_padding_mask)[0]
        text_info_fact2 = self.img2text_attn_fact2(query=text_info_fact1, key=self.with_pos_embed(cate_asso_sent2, word_pos), value=cate_asso_sent2, key_padding_mask=word_key_padding_mask)[0]
        text_info_fact3 = self.img2text_attn_fact3(query=text_info_fact2, key=self.with_pos_embed(cate_asso_sent3, word_pos), value=cate_asso_sent3, key_padding_mask=word_key_padding_mask)[0]

        text_embed_fact1 = self.text_proj_fact1(text_info_fact1)
        text_embed_fact2 = self.text_proj_fact2(text_info_fact2)
        text_embed_fact3 = self.text_proj_fact3(text_info_fact3)
        img_embed = self.img_proj(img_feat)
        verify_score = (F.normalize(img_embed, p=2, dim=-1) *
                        F.normalize(text_embed_fact1, p=2, dim=-1)).sum(dim=-1, keepdim=True)
        verify_score_all = self.tf_scale * \
                       torch.exp( - (1 - verify_score).pow(self.tf_pow) \
                        / (2 * self.tf_sigma**2))
        verify_score = (F.normalize(img_embed, p=2, dim=-1) * F.normalize(text_embed_fact2, p=2, dim=-1)).sum(dim=-1, keepdim=True)
        verify_score_all += self.tf_scale_fact2 * torch.exp(- (1 - verify_score).pow(self.tf_pow) / (2 * self.tf_sigma_fact2 ** 2))
        verify_score = (F.normalize(img_embed, p=2, dim=-1) * F.normalize(text_embed_fact3, p=2, dim=-1)).sum(dim=-1, keepdim=True)
        verify_score_all += self.tf_scale_fact3 * torch.exp(- (1 - verify_score).pow(self.tf_pow) / (2 * self.tf_sigma_fact3 ** 2))
        verify_score = verify_score_all

        # 2nd branch
        text_cond_info_fact1 = self.img2textcond_attn_fact1(query=img_feat, key=self.with_pos_embed(cate_asso_sent1, word_pos), value=cate_asso_sent1, key_padding_mask=word_key_padding_mask)[0]
        text_cond_info_fact2 = self.img2textcond_attn_fact2(query=text_cond_info_fact1, key=self.with_pos_embed(cate_asso_sent2, word_pos), value=cate_asso_sent2, key_padding_mask=word_key_padding_mask)[0]
        text_cond_info_fact3 = self.img2textcond_attn_fact3(query=text_cond_info_fact2, key=self.with_pos_embed(cate_asso_sent3, word_pos), value=cate_asso_sent3, key_padding_mask=word_key_padding_mask)[0]
        text_cond_info = torch.cat([text_cond_info_fact1, text_cond_info_fact2, text_cond_info_fact3], dim=-1)
        text_cond_info = self.merge(text_cond_info)
        ...
        fuse_img_feat = (self.norm_img(img_feat) + self.norm_text_cond_img(text_cond_img_ctx)) * verify_score
