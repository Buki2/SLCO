...
def get_ranking_return_top1(log_probs, masked_indices, vocab, label_index = None, index_list = None, topk = 1000, P_AT = 10, print_generation=True):
    ...
    ## return fact obj label
    fact_obj_label = result_masked_topk[0]['token_word_form']
    return fact_obj_label


def get_ranking_return_top1_v2(log_probs, masked_indices, vocab, label_index = None, index_list = None, topk = 1000, P_AT = 10, print_generation=True, original_text = None):

    experiment_result = {}

    topk = log_probs.size(-1)
    ...
    fact_obj_label = 0
    i = 0
    while not fact_obj_label:
        if i < len(result_masked_topk):
            cand_label = result_masked_topk[i]['token_word_form']
        else:
            fact_obj_label = cand_label
            break
        if cand_label not in original_text:
            fact_obj_label = cand_label
        i += 1
    return fact_obj_label


def get_ranking_return_all(log_probs, masked_indices, vocab, label_index = None, index_list = None, topk = 1000, P_AT = 10, print_generation=True, original_text = None):
    ...
    fact_obj_label_all = [result_masked_topk[i]['token_word_form'] for i in range(len(result_masked_topk))]

    return fact_obj_label_all
...
def get_ranking(log_probs, masked_indices, vocab, label_index = None, index_list = None, topk = 1000, P_AT = 10, print_generation=True):
    ...
    log_probs, index_max_probs, value_max_probs = __max_probs_values_indices(masked_indices, log_probs, topk=topk)
    result_masked_topk, return_msg = __print_top_k(value_max_probs, index_max_probs, vocab, topk, index_list, max_printouts=topk)
    experiment_result['topk'] = result_masked_topk
